...
def fetch_osm_venue(category_key, city):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            named_elements = [e for e in elements if "tags" in e and "name" in e["tags"]]
            if named_elements:
...
"""
    ...
    try:
        ...
        if "COVER_TITLE:" in full_text:
            ...
            cover_title = parts[0].strip().replace("[", "").replace("]", "").replace("*", "")
        ...
        return f"{category_key} SPOTLIGHT", "XGAME RADAR", f"👋 我係 Una (@Una_next)！今日最新極限情報更新～\n\n#xGameRadar", cat_info['query']
...
def create_obsidian_background(width=1200, height=630):
    ...
    draw_overlay.ellipse([width - 350, height - 350, width + 100, height + 100], fill=(0, 212, 255, 25))
...
def create_cover_image(cover_title, sub_title, query_keyword, output_path="cover.jpg"):
    ...
    draw.text((80, 530), "xGame Radar · Curated by Una (@Una_next)", font=font_footer, fill=(200, 200, 210))
...
def main():
    ...
    parser.add_argument("-c", "--category", default="AUTO", choices=["AUTO", "SKATE", "SURF", "CLIMB", "EVENT"], help="主題類別")
    parser.add_argument("-l", "--lang", default="zh-hk", choices=["zh-hk", "zh-cn", "ja", "en"], help="發布語言")
    ...
    photo_path = create_cover_image(cover_title, sub_title, query_keyword, "xgame_post.jpg")
